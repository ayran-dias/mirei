...
function RoadmapCardNode({ id, data, selected }: NodeProps) {
  ...
  const colorOpacity = (d as unknown as Record<string, unknown>).colorOpacity as string | undefined
  const opacityHex = colorOpacity ?? 'cc'  // default ~80% opacity
  const shapeFill   = customColor ? customColor + opacityHex : hex.fill
  const shapeStroke = customColor || hex.stroke
  const contentInset = SHAPE_INSET[shape] ?? SHAPE_INSET.rect
  const fontColor = d.fontColor
  const isBold = d.bold ?? false
  const isItalic = d.italic ?? false
    ...
    <>
      ...
      <NodeToolbar isVisible={!!selected && !!actions} position={Position.Top}>
        <div className="flex items-center gap-0.5 bg-white border border-gray-200 rounded-lg shadow-lg px-1.5 py-1">
          ...
          <button
            ref={fontColorBtnRef}
            onClick={() => fontColorBtnRef.current && actions.onOpenColorPicker(id, fontColorBtnRef.current.getBoundingClientRect(), 'fontColor')}
            title="Cor da fonte"
            className="w-6 h-6 flex items-center justify-center rounded hover:bg-gray-100 transition-colors"
          >
            <span className="w-3.5 h-3.5 flex items-center justify-center text-[10px] font-black ring-1 ring-black/15 rounded-full" style={{ color: fontColor || '#1A1A1A', background: '#f5f5f5' }}>A</span>
          </button>
          <button
            onClick={() => actions.onStatusChange(id, '__toggle_bold__')}
            title="Negrito"
            className={`w-6 h-6 flex items-center justify-center rounded hover:bg-gray-100 transition-colors text-[11px] ${isBold ? 'bg-gray-200 font-black text-gray-800' : 'font-bold text-gray-400'}`}
          >B</button>
          <button
            onClick={() => actions.onStatusChange(id, '__toggle_italic__')}
            title="Itálico"
            className={`w-6 h-6 flex items-center justify-center rounded hover:bg-gray-100 transition-colors text-[11px] italic ${isItalic ? 'bg-gray-200 text-gray-800' : 'text-gray-400'}`}
          >I</button>
          <button
      ...
      <div
        ...
        style={{
          ...
          ...(!isShaped && customColor ? { borderColor: customColor, backgroundColor: customColor + opacityHex } : {}),
        }}
      ...
      >
        ...
        >
          ...
          <div className="flex flex-col flex-1 min-h-0 overflow-hidden" style={{ justifyContent }}>
            <p
              className="text-sm leading-tight mt-1.5"
              style={{
                textAlign,
                color: fontColor || (isDone ? '#c7ff3d' : '#1A1A1A'),
            ...
            {expanded && d.description && (
              <p
                ...
                style={{
                  textAlign,
                  color: fontColor ? fontColor + 'b3' : (isDone ? 'rgba(255,255,255,0.6)' : '#6b7280'),
                  fontStyle: isItalic ? 'italic' : undefined,
                }}
              >
                {d.description}
