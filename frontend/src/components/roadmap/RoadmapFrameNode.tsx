...
function RoadmapFrameNode({ id, data, selected }: NodeProps) {
  ...
  const commitTitle = useCallback(() => {
    if (!actions) return
    const trimmed = titleDraft.trim() || 'Frame'
    setEditingTitle(false)
    actions.onFrameTitleChange(id, trimmed, getNodes(), setNodes, sizeDraft)
  ...
  return (
    <>
      ...
      {/* Outer container — pointer-events: none on body so edges below are clickable */}
      <div
        className="w-full h-full flex flex-col font-['Manrope',sans-serif]"
        onMouseEnter={() => setHovered(true)}
        onMouseLeave={() => setHovered(false)}
        style={{
          border: selected
            ? `2px solid ${headerColor}66`
            : `2px dashed ${headerColor}40`,
          borderRadius: 8,
          background: isMinimized ? '#00461e' : 'rgba(255,255,255,0.03)',
          overflow: 'hidden',
          pointerEvents: 'none',
        }}
      >
        {/* Header bar — keeps pointer-events for dragging and interaction */}
        <div
          className="flex items-center justify-between px-3 shrink-0 cursor-grab active:cursor-grabbing"
          style={{
            minHeight: HEADER_HEIGHT,
            padding: '6px 12px',
            background: headerColor,
            borderRadius: isMinimized ? '4px' : '4px 4px 0 0',
            pointerEvents: 'auto',
          }}
        >
          {editingTitle ? (
            <div className="nodrag nopan flex items-center flex-1 gap-1.5 min-w-0 mr-2">
              <input
                ...
                style={{ fontFamily: 'Manrope, sans-serif', fontSize: sizeDraft }}
            ...
            <span
              className="flex-1 text-white font-bold truncate select-none mr-2"
              style={{ fontFamily: 'Manrope, sans-serif', letterSpacing: '-0.01em', fontSize: d.titleSize ?? 20 }}
              onDoubleClick={handleTitleDoubleClick}
              title={actions ? 'Duplo clique para editar' : undefined}
            >
              {d.title}
            </span>
          )}
          ...
          {actions && (
            <div className="flex items-center gap-1 shrink-0">
              <button
                ...
                title="Cor do frame"
                className="nodrag nopan w-5 h-5 flex items-center justify-center rounded hover:bg-white/20 transition-colors"
              >
        ...
        {/* Body area — only visible when maximized */}
        {!isMinimized && (
