...
import RoadmapGroupNode, { type RoadmapGroupData } from './RoadmapGroupNode'
...
function LabelModal({ open, initial, onSave, onDelete, onClose }: {
  ...
  // Sync state when a different label is opened for editing
  useEffect(() => {
    if (open && initial) {
      setText(initial.text)
      setFontSize(initial.fontSize || 24)
...
// ── TableConfigModal ──────────────────────────────────────────────────────────
function TableConfigModal({ open, atPosition, initial, onSave, onClose }: {
  open: boolean
  atPosition: { x: number; y: number } | null
  initial?: RoadmapTableData | null
  onSave: (data: RoadmapTableData, pos?: { x: number; y: number }) => void
  onClose: () => void
}) {
  const isEdit = !!initial
  const [title, setTitle] = useState('')
  const [columns, setColumns] = useState<{ header: string; hasCheckbox: boolean; options?: string[]; optionColors?: Record<string, string> }[]>([
    { header: 'Item', hasCheckbox: false },
    { header: 'Status', hasCheckbox: true, options: ['Em andamento', 'Concluído', 'Pendente'] },
  ])
  ...
  useEffect(() => {
    if (open && initial) {
      ...
      setColumns(initial.columns.map(c => ({ header: c.header, hasCheckbox: c.hasCheckbox, options: c.options, optionColors: c.optionColors })))
    } else if (open) {
      setTitle('')
      setColumns([
        { header: 'Item', hasCheckbox: false },
  ...
  const updateOptions = (i: number, raw: string) =>
    setColumns(c => c.map((col, ci) => ci === i ? { ...col, options: raw.trim() ? raw.split(',').map(s => s.trim()) : undefined } : col))
  ...
  const handleCreate = () => {
    let rows: RoadmapTableData['rows']
    if (isEdit && initial) {
      // Preserve existing rows, adjusting cells to match new column count
      rows = initial.rows.map(r => ({
        ...r,
        cells: columns.map((col, ci) => r.cells[ci] ?? { text: '', checked: col.hasCheckbox ? false : undefined }),
      }))
    } else {
      rows = [
    ...
    const extra = isEdit ? { headerColor: initial!.headerColor, headerFontColor: initial!.headerFontColor, stripeColor: initial!.stripeColor, fontColor: initial!.fontColor } : {}
    onSave({ title, columns, rows, ...extra }, atPosition ?? undefined)
  }

  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/40 backdrop-blur-sm" onClick={onClose}>
      <div className="bg-white rounded-2xl shadow-xl border border-gray-200 w-full max-w-lg mx-4 max-h-[85vh] flex flex-col" onClick={e => e.stopPropagation()}>
        <div className="bg-[#00461e] px-5 py-3 flex items-center justify-between shrink-0">
          <h3 className="font-bold text-white text-sm">{isEdit ? 'Editar tabela' : 'Nova tabela'}</h3>
          <button onClick={onClose} className="text-white/50 hover:text-white">
            <svg className="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M6 18L18 6M6 6l12 12" /></svg>
          </button>
        </div>
              ...
              {columns.map((col, i) => (
                <div key={i} className="space-y-1">
                  ...
                  <input
                    ...
                    className="w-full border border-gray-100 rounded-lg px-2 py-1 text-[10px] text-gray-500 focus:outline-none focus:ring-1 focus:ring-[#00461e]/20"
                    placeholder="Opções dropdown (separar por vírgula)"
                  />
                  {/* Option colors */}
                  {col.options && col.options.length > 0 && (
                    <div className="flex flex-wrap gap-1 mt-1">
                      {col.options.map(opt => (
                        <label key={opt} className="flex items-center gap-1 text-[9px] text-gray-500">
                          <input
                            type="color"
                            value={col.optionColors?.[opt] || '#ffffff'}
                            onChange={e => {
                              const colors = { ...(col.optionColors || {}) }
                              if (e.target.value === '#ffffff') delete colors[opt]; else colors[opt] = e.target.value
                              setColumns(c => c.map((cc, ci) => ci === i ? { ...cc, optionColors: Object.keys(colors).length ? colors : undefined } : cc))
                            }}
                            className="w-4 h-4 rounded cursor-pointer border border-gray-200"
                          />
...
function Canvas({ initialNodes, initialEdges, isEditor: isEditorProp, initialViewport, initialCustomColors }: Props) {
  ...
  const [editTableNode, setEditTableNode] = useState<Node | null>(null)
  ...
  // Merge measured dimensions into style so reload reproduces exact sizes
  const buildPayload = useCallback((n: Node[], e: Edge[]) => JSON.stringify({
    nodes: n.map(nd => {
      const mw = (nd as any).measured?.width as number | undefined
      const mh = (nd as any).measured?.height as number | undefined
      const style = { ...(nd.style || {}) } as Record<string, unknown>
      if (mw != null) style.width = mw
      if (mh != null) style.height = mh
      return { id: nd.id, type: nd.type, position: nd.position, data: nd.data, style, zIndex: nd.zIndex, hidden: nd.hidden }
    }),
  ...
  // Save when edges change externally (e.g. waypoint drag in RoadmapEdge)
  const edgesJsonRef = useRef('')
  useEffect(() => {
    if (!isEditor) return
    const json = JSON.stringify(edges.map(e => e.data))
    if (edgesJsonRef.current && json !== edgesJsonRef.current) {
      triggerSave(nodes, edges)
    }
    edgesJsonRef.current = json
  }, [edges, nodes, isEditor, triggerSave])

  const handleGroup = useCallback(() => {
    const sel = nodes.filter(n => n.selected && n.type !== 'group' && n.type !== 'frame')
    ...
    const groupNode: Node = {
      ...
      data: { containedNodeIds: sel.map(n => n.id) } as unknown as Record<string, unknown>,
      style: { width: maxX - minX + PAD, height: maxY - minY + PAD },
      zIndex: Math.min(-1, Math.min(...sel.map(n => n.zIndex ?? 0)) - 1),
    }
    setNodes(ns => {
      const updated = [...ns, groupNode].map(n => ({ ...n, selected: false }))
  ...
  const handleColorChange = useCallback((targetId: string, targetType: 'node' | 'edge', color: string | null, colorKey?: string) => {
  ...
  const handleNodesChange: OnNodesChange = useCallback((changes) => {
    ...
    // Include 'dimensions' changes — NodeResizer fires these when user resizes
    const hasUserChange = changes.some(c => c.type !== 'select')
    if (hasUserChange) {
      setTimeout(() => {
  ...
  const onNodeDoubleClick = useCallback((_: React.MouseEvent, node: Node) => {
    ...
    else if (node.type === 'table') { setEditTableNode(node) }
  ...
  const handleEditTable = useCallback((data: RoadmapTableData) => {
    if (!editTableNode) return
    setNodes(ns => {
      const updated = ns.map(n => n.id === editTableNode.id ? { ...n, data: data as unknown as Record<string, unknown> } : n)
      triggerSave(updated, edges)
      return updated
    })
    setEditTableNode(null)
  }, [editTableNode, setNodes, edges, triggerSave])
  ...
  const handleAddCard = useCallback((data: RoadmapCardData) => {
    ...
    const newNode: Node = { id, type: 'card', position: pos, data, style: { width: 230, height: 120 } }
    setNodes(ns => {
      const updated = [...ns, newNode]
      triggerSave(updated, edges)
      return updated
  ...
  const handleStatusChange = useCallback((id: string, status: string) => {
    setNodes(ns => {
      const updated = ns.map(n => {
        ...
        if (status === '__toggle_bold__') return { ...n, data: { ...d, bold: !d.bold } }
        if (status === '__toggle_italic__') return { ...n, data: { ...d, italic: !d.italic } }
  ...
  const onNodeDrag = useCallback((_: React.MouseEvent, node: Node) => {
    if ((node.type !== 'frame' && node.type !== 'group') || !dragStart.current || dragStart.current.id !== node.id) return
  ...
  const handleUngroup = useCallback((groupId: string) => {
    setNodes(ns => {
      const updated = ns.filter(n => n.id !== groupId)
      triggerSave(updated, edges)
      return updated
    })
  }, [setNodes, edges, triggerSave])
  ...
  }), [handleToolbarEdit, handleToolbarDelete, handleStatusChange, handleFrameToggle, handleFrameTitleChange, handleTableChange, handleOpenColorPicker, handleUngroup])
    ...
    <RoadmapActionsContext.Provider value={isEditor ? roadmapActions : null as unknown as RoadmapActions}>
      <div className="relative w-full" style={{ height: '100%' }} ref={flowRef}>
        ...
        <TableConfigModal
          open={!!editTableNode}
          atPosition={null}
          initial={editTableNode ? (editTableNode.data as unknown as RoadmapTableData) : null}
          onSave={handleEditTable}
          onClose={() => setEditTableNode(null)}
        />
        ...
        {colorPickerTarget && (
          <ColorPicker
            ...
            onSelect={color => handleColorChange(colorPickerTarget.targetId, colorPickerTarget.targetType, color, colorPickerTarget.colorKey)}
