...
function FB({ runOpts, onApply, onFilteredDocs }: {
  ...
  const { data: rawOpts, loading: optsLoading, run: fetchOpts } = useGASLazy<FiltOpt>('getGMFilterOptions')
  ...
  return (
    <div className="mb-4 p-3 bg-[#f5fff5] rounded-xl border border-[#c8d2c8]">
      ...
      <div className="grid grid-cols-2 sm:grid-cols-4 lg:grid-cols-7 gap-2 mb-3">
        ...
        {field('Categoria',         'grupo1s', 'grupo1')}
        {field('Responsável (ID)',  'grupo2s', 'grupo2')}
...
// Hook: retorna apenas colunas com pelo menos 1 valor não-zero e não-nulo
...
function C1({ onFilteredDocs, onActiveFilters }: { onFilteredDocs?: (docs: string[]) => void; onActiveFilters?: (f: Record<string, string[]>) => void }) {
  const { data, loading, run } = useGASLazy<BaseGeralRow>('getGMBaseGeral')
  useEffect(() => { run(); if (onActiveFilters) onActiveFilters({}) }, []) // eslint-disable-line — trigger initial summary with empty filters
  // TPV Boleto, TPV Sub e TPV GTW desativados por default no Grupos Marca
  const GM_EXCLUDED = new Set(['tpv_boleto', 'tpv_sub', 'tpv_gtw'])
  const [sel, setSel] = useState<Set<string>>(new Set(ALL_BG_COLS.map(([k]) => k as string).filter(k => !GM_EXCLUDED.has(k))))
...
function C2({ opts }: { opts: string[] }) {
  const { data, loading } = useGAS<Visao3MRow>('getGMVisao3M')
  const ml = getML()
...
function C3({ opts }: { opts: string[] }) {
  const { data, loading, run } = useGASLazy<TransRow>('getGMTransacional')
  const [gqs, setGqs] = useState<string[]>([])
...
function C4({ opts }: { opts: string[] }) {
  const { data, loading, error } = useGAS<TransGRow>('getGMTransacionalGrupos')
  const [gqs, setGqs] = useState<string[]>([])
  const [page, setPage] = useState(0)
...
function C5({ opts, mesOpts }: { opts: string[]; mesOpts: string[] }) {
  const { data: canalData, loading: canalLoading, run: runCanal } = useGASLazy<RAVRow>('getGMRAV')
  const { data: clienteData, loading: clienteLoading, run: runCliente } = useGASLazy<RAVRow>('getGMRAVCliente')
  const [gqs, setGqs] = useState<string[]>([])
...
function C6({ opts }: { opts: Record<string, string[]> }) {
  const { data, loading, run } = useGASLazy<ModalRow>('getGMModalidade')
  const [gqs, setGqs] = useState<string[]>([])
...
function C7({ opts, mesOpts }: { opts: string[]; mesOpts: string[] }) {
  const { data, loading, error, run } = useGASLazy<MetRow>('getGMMetricasCliente')
  const [mes, setMes] = useState('')
...
function C8({ opts }: { opts: string[] }) {
  const { data, loading, run } = useGASLazy<LRRow>('getGMLinhasReceita')
  const [gqs, setGqs] = useState<string[]>([])
  useEffect(() => { run() }, []) // eslint-disable-line
...
function C9({ opts, mesOpts }: { opts: string[]; mesOpts: string[] }) {
  const { data, loading, run } = useGASLazy<TPRow>('getGMTodosProdutos')
  const [gqs, setGqs] = useState<string[]>([])
...
function C10({ opts }: { opts: string[] }) {
  ...
  const { data, loading, error, run } = useGASLazy<AfilRow>('getGMAfiliacoes')
  const [gqs, setGqs] = useState<string[]>([])
  const [page, setPage] = useState(0)
  useEffect(() => setPage(0), [gqs])

  // getGMAfiliacoes não tem filtro server-side por grupo — client-side é suficiente
  const filtered = (data || []).filter(r => gqs.length === 0 || gqs.includes(r.grupo))
  const paginated = filtered.slice(page * PG, (page + 1) * PG)
  const thC = "px-2 py-2 text-right font-semibold text-[#505a50] text-[11px] whitespace-nowrap"
...
export default function Enterprise() {
  ...
  const { data: ro } = useGAS<FiltOpt>('getGMFilterOptions')
  ...
  return (
    <div>
      ...
      <AnimatedHero className="px-8 py-16">
        <div className="max-w-7xl mx-auto">
          <div className="flex items-center gap-2 text-xs mb-4" style={{ color: 'rgba(255,255,255,0.5)' }}>
            ...
            <span className="font-bold" style={{ color: 'rgba(255,255,255,0.8)' }}>Grupos Marca</span>
          </div>
          <h1 className="font-extrabold leading-[1.05] mb-4" style={{ fontSize: 'clamp(2rem, 5vw, 3.5rem)', color: 'white' }}>
            Grupos Marca
          </h1>
          <p className="text-base max-w-xl leading-relaxed" style={{ color: 'rgba(255,255,255,0.9)' }}>
            <span className="font-bold">Carteira: Grupos Marca</span><br />
            Acompanhamento da carteira por grupo de marca.
          </p>
        </div>
      </AnimatedHero>

      {/* Aviso de cobertura — Pagar.me ausente no PnL_GM */}
      ...
      <div className="space-y-4">
        ...
        <CardCreditoLifetime carteira="gm" filters={activeFilters} groupOpts={opts.grupo || []} />
